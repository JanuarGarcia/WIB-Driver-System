...
- **UUID discovery for CEO / Postman env setup**: run `npm run smoke-mangan-bearer -- <username> <password> <baseUrl> /driver/profile`. The script now logs a JSON block with `baseUrl`, `userToken`, `driverUuid`, and `scheduleUuid`. By default it discovers `driver_uuid` from `/driver/profile` and `schedule_uuid` from `/driver/shift`. If the live Mangan API uses different paths or request bodies, override them with `MANGAN_PROFILE_PATH`, `MANGAN_PROFILE_METHOD`, `MANGAN_PROFILE_BODY`, `MANGAN_SHIFT_PATH`, `MANGAN_SHIFT_METHOD`, and `MANGAN_SHIFT_BODY`.
