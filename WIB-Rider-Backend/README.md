...
- **UUID discovery for CEO / Postman env setup**: run `npm run smoke-mangan-bearer -- <username> <password> <baseUrl> /driver/profile`. The script now logs a JSON block with `baseUrl`, `userToken`, `driverUuid`, and `scheduleUuid`. It follows the Postman collection more closely: `driver_uuid` comes from `/driver/profile`, then `schedule_uuid` is attempted from `/driver/getshift` with fallback to `/driver/currentShift`. If the live Mangan API uses different paths or request bodies, override them with `MANGAN_PROFILE_PATH`, `MANGAN_PROFILE_METHOD`, `MANGAN_PROFILE_BODY`, `MANGAN_SHIFT_PATH`, `MANGAN_SHIFT_METHOD`, `MANGAN_SHIFT_BODY`, `MANGAN_CURRENT_SHIFT_PATH`, `MANGAN_CURRENT_SHIFT_METHOD`, and `MANGAN_CURRENT_SHIFT_BODY`.
...
- **Optional TIN ID**: run `sql/2026-05-07_mt_driver_tin_id.sql` to add `mt_driver.mt_tin_id` (nullable). JSON key is `tin_id` (input/output); backend also accepts legacy `mt_tin_id` on requests.
