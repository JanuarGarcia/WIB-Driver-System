...
export default function MapApiLogs() {
  ...
  const fetchLogs = () => {
    setLoading(true);
    api('map-api-logs').then((data) => setLogs(Array.isArray(data) ? data : data?.logs || [])).catch(() => setLogs([])).finally(() => setLoading(false));
  };
  ...
  const sortedLogs = useTableSort(logs || [], sortKey, sortOrder, MAPLOG_SORT_OPTIONS);
  const {
    ...
    <div className="listing-section">
      ...
      {!loading && (
        <div className="listing-table-card">
          <TableSortControls
            sortOptions={MAPLOG_SORT_OPTIONS.map(({ key, label }) => ({ key, label }))}
            sortKey={sortKey}
            sortOrder={sortOrder}
            ...
            <table>
              ...
              <tbody>
                ...
                <tr key={l.id}>
                  ...
                  <td>{l.provider ?? '—'}</td>
                  <td>{l.api ?? '—'}</td>
                  <td>{(l.response ?? '').slice(0, 80)}{(l.response ?? '').length > 80 ? '…' : ''}</td>
                  <td>{formatDate(l.date_created ?? l.date)}</td>
                </tr>
                ))}
              </tbody>
            </table>
            {(!logs || logs.length === 0) && <p className="listing-empty-msg">No map API logs.</p>}
          </div>
