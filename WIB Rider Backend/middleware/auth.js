...
async function resolveDriver(req, res, next) {
  const token = req.query?.token || req.body?.token || (req.headers.authorization && req.headers.authorization.replace(/^Bearer\s+/i, ''));
  if (!token) {
...
module.exports = { validateApiKey, resolveDriver, optionalDriver, getApiKey };
